...
use App\Models\Escalafon\VMantenimientoMovimientoAccion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoMovimientoAccionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoMovimientoAccion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoMovimientoAccion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::beginTransaction();
            $resource = VMantenimientoMovimientoAccion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoMovimientoAccion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
