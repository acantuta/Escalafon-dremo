...
use App\Models\Escalafon\TipoDocumentoIdentificacion;
...
class TipoDocumentoIdentificacionController extends Controller
{
    ...
    {
        try {    
            $query = TipoDocumentoIdentificacion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoDocumentoIdentificacion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipoDocIdenNombre' => 'nullable|string|max:150',
            'iTipoIdentId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = TipoDocumentoIdentificacion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoDocumentoIdentificacion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
