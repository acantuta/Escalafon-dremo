...
use App\Models\Escalafon\EducacionCentroRegistro;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionCentroRegistroController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionCentroRegistro::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionCentroRegistro::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'iEduCentRegNombre' => 'nullable|string|max:250',

            ]);

            DB::beginTransaction();
            $resource = EducacionCentroRegistro::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionCentroRegistro::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
