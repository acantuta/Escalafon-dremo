...
use App\Models\Escalafon\Departamento;
...
class DepartamentoController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = Departamento::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cDptoCodigo' => 'nullable|string|max:10',
            'cDptoNombre' => 'nullable|string|max:100',
            'cDptoAbreviatura' => 'nullable|string|max:10',
            'iDptoEstado' => 'nullable|integer',
            'cDptoGentilicio' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = Departamento::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = Departamento::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
