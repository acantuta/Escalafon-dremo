...
class VMovimientoPersonalController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMovimientoPersonal::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMovimientoPersonal::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cVacLicNumeroDocumento' => 'nullable|string|max:150',
            'dtVacLicFechaDocumento' => 'nullable|date',
            'iArchId' => 'nullable|integer',
            'iRegLabId' => 'nullable|integer',
            'iMovAccId' => 'nullable|integer',
            'iMovMotId' => 'nullable|integer',
            'dtVacLicFechaInicio' => 'nullable|date',
            'dtVacLicFechaFin' => 'nullable|date',
            'cVacLicAnotaciones' => 'nullable|string',
            'cMovAccNombre' => 'nullable|string|max:150',
            'cMovMotNombre' => 'required|string|max:150',
            'cRegLabNombre' => 'nullable|string|max:300',
            'iVacLicId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VMovimientoPersonal::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMovimientoPersonal::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
