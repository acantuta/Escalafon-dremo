...
use App\Models\Escalafon\TipoServidor;
...
class TipoServidorController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoServidor::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipoSerNombre' => 'required|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = TipoServidor::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoServidor::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
