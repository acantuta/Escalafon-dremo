...
use App\Models\Escalafon\TipoComisionPensionario;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class TipoComisionPensionarioController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = TipoComisionPensionario::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoComisionPensionario::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'TipComPenNombre' => 'nullable|string|max:250',

            ]);

            DB::beginTransaction();
            $resource = TipoComisionPensionario::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoComisionPensionario::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
