...
use App\Models\Escalafon\VMantenimientoEducacionPrograma;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEducacionProgramaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEducacionPrograma::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEducacionPrograma::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iEduProId' => 'required|integer',
            'cEduProProfNombre' => 'nullable|string|max:200',
            'cEduProNombre' => 'nullable|string|max:150',
            'iEduProProfId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEducacionPrograma::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEducacionPrograma::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
