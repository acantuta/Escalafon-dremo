...
use App\Models\Escalafon\AcreditacionLenguaNativa;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class AcreditacionLenguaNativaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = AcreditacionLenguaNativa::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = AcreditacionLenguaNativa::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cAcredIdioma' => 'nullable|string|max:100',
            'cAcredDominioEscritura' => 'nullable|string|max:100',
            'cAcredDominioOral' => 'nullable|string|max:100',
            'iAcredAnioIngreso' => 'nullable|integer',
            'iAcredAnioEvaluacion' => 'nullable|integer',
            'iAcredAnioVencimiento' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = AcreditacionLenguaNativa::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = AcreditacionLenguaNativa::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
