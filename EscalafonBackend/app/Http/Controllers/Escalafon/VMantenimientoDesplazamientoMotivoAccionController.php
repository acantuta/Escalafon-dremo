...
use App\Models\Escalafon\VMantenimientoDesplazamientoMotivoAccion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoDesplazamientoMotivoAccionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoDesplazamientoMotivoAccion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoDesplazamientoMotivoAccion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iDespMotAccId' => 'required|integer',
            'cDespNombre' => 'required|string|max:150',
            'cDespAccNombre' => 'required|string|max:150',
            'iDespAccId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoDesplazamientoMotivoAccion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoDesplazamientoMotivoAccion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
