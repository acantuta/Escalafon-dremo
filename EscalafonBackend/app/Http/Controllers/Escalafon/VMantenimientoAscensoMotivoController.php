...
use App\Models\Escalafon\VMantenimientoAscensoMotivo;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoAscensoMotivoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoAscensoMotivo::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoAscensoMotivo::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iAscMotId' => 'required|integer',
            'cAscMotNombre' => 'required|string|max:150',
            'iAscAccId' => 'nullable|integer',
            'cAscAccNombre' => 'required|string|max:150',
            ...
            DB::beginTransaction();
            $resource = VMantenimientoAscensoMotivo::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoAscensoMotivo::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
