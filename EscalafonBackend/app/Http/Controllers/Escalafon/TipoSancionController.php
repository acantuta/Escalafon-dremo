...
use App\Models\Escalafon\TipoSancion;
...
class TipoSancionController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoSancion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipSancNombre' => 'nullable|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = TipoSancion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoSancion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
