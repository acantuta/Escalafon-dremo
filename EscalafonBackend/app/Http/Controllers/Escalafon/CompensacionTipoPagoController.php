...
use App\Models\Escalafon\CompensacionTipoPago;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class CompensacionTipoPagoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = CompensacionTipoPago::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = CompensacionTipoPago::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cCompTipPagNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = CompensacionTipoPago::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = CompensacionTipoPago::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
