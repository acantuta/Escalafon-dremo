...
class VInfopefamiliarFamiliarController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VInfopefamiliarFamiliar::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VInfopefamiliarFamiliar::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iInfoPeFamFamiId' => 'required|integer',
            ...
            'cTipoIdentNombre' => 'nullable|string|max:50',
            'bInfoPeFamFamiEsFallecido' => 'nullable',

            ]);

            DB::beginTransaction();
            $resource = VInfopefamiliarFamiliar::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VInfopefamiliarFamiliar::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
