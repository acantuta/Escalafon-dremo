...
use App\Models\Escalafon\ReconocimientoTipoMerito;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class ReconocimientoTipoMeritoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = ReconocimientoTipoMerito::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = ReconocimientoTipoMerito::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cRecoTipMerNombre' => 'required|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = ReconocimientoTipoMerito::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = ReconocimientoTipoMerito::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
