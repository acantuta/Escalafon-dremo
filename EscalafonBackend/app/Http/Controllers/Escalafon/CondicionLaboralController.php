...
use App\Models\Escalafon\CondicionLaboral;
...
class CondicionLaboralController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = CondicionLaboral::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iGrupoPrestadorId' => 'nullable|integer',
            'cCondLabNombre' => 'nullable|string|max:50',
            'cCondLabCodigoSunat' => 'nullable|string|max:2',
            'cCondLabCodigoAirshp' => 'nullable|string|max:2',

            ]);

            DB::beginTransaction();
            $resource = CondicionLaboral::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = CondicionLaboral::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
