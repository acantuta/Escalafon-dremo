...
use App\Models\Escalafon\VMantenimientoCategoriaRemunerativa;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoCategoriaRemunerativaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoCategoriaRemunerativa::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoCategoriaRemunerativa::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iCatRemuId' => 'required|integer',
            'cCatRemuNombre' => 'nullable|string|max:250',
            'iGrupOcupId' => 'nullable|integer',
            'cGrupOcupNombre' => 'nullable|string|max:250',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoCategoriaRemunerativa::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoCategoriaRemunerativa::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
