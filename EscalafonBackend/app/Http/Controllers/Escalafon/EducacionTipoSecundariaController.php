...
use App\Models\Escalafon\EducacionTipoSecundaria;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionTipoSecundariaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionTipoSecundaria::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionTipoSecundaria::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduTipoSecId' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = EducacionTipoSecundaria::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionTipoSecundaria::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
