...
class VSistemaPensionarioController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VSistemaPensionario::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VSistemaPensionario::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iAdmFonPenId' => 'nullable|integer',
            'iTipComPenId' => 'nullable|integer',
            'iArchId' => 'required|integer',
            'dtSisPenFechaVigencia' => 'nullable|date',
            'dtSisPenFechaAfiliacion' => 'nullable|date',
            'dtSisPenFechaDevengue' => 'nullable|date',
            'cSisPenNumeroCuspp' => 'nullable|string|max:50',
            'cSisPenAnotaciones' => 'nullable|string',
            'iRegPenId' => 'required|integer',
            'cRegPenNombre' => 'nullable|string|max:150',
            'cAdmFonPenNombre' => 'nullable|string|max:250',
            'iSisPenId' => 'required|integer',
            'iLegId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VSistemaPensionario::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VSistemaPensionario::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
