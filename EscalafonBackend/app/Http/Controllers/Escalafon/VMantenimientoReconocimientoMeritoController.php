...
use App\Models\Escalafon\VMantenimientoReconocimientoMerito;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoReconocimientoMeritoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoReconocimientoMerito::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoReconocimientoMerito::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cRecoMerNombre' => 'required|string|max:150',
            'iRecoTipMerId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoReconocimientoMerito::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoReconocimientoMerito::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
