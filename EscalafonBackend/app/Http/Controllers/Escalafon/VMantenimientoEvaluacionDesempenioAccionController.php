...
use App\Models\Escalafon\VMantenimientoEvaluacionDesempenioAccion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEvaluacionDesempenioAccionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEvaluacionDesempenioAccion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEvaluacionDesempenioAccion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cEvalDesAccNombre' => 'required|string|max:150',
            'iRegLabId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEvaluacionDesempenioAccion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEvaluacionDesempenioAccion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
