...
use App\Models\Escalafon\EducacionTipoParticipacion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionTipoParticipacionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionTipoParticipacion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionTipoParticipacion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduTipPartNombre' => 'required|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = EducacionTipoParticipacion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionTipoParticipacion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
