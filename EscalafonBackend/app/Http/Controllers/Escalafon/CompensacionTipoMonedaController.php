...
use App\Models\Escalafon\CompensacionTipoMoneda;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class CompensacionTipoMonedaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = CompensacionTipoMoneda::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = CompensacionTipoMoneda::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cComTipMonNombre' => 'nullable|string|max:50',

            ]);

            DB::beginTransaction();
            $resource = CompensacionTipoMoneda::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = CompensacionTipoMoneda::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
