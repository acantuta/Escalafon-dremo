...
use App\Models\Escalafon\VMantenimientoEducacionProgramaProfesional;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEducacionProgramaProfesionalController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEducacionProgramaProfesional::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEducacionProgramaProfesional::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iEduProProfId' => 'required|integer',
            'cEduProGenNombre' => 'nullable|string|max:150',
            'cEduProProfNombre' => 'nullable|string|max:200',
            'iEduProGenId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEducacionProgramaProfesional::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEducacionProgramaProfesional::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
