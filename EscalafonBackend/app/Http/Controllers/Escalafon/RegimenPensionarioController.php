...
use App\Models\Escalafon\RegimenPensionario;
...
class RegimenPensionarioController extends Controller
{
    ...
    {
        try {    
            $query = RegimenPensionario::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = RegimenPensionario::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cRegPenNombre' => 'required|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = RegimenPensionario::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = RegimenPensionario::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
