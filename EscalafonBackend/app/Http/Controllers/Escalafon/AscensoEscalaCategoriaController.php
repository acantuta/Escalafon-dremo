...
use App\Models\Escalafon\AscensoEscalaCategoria;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class AscensoEscalaCategoriaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = AscensoEscalaCategoria::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = AscensoEscalaCategoria::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cAscEscCatNombre' => 'required|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = AscensoEscalaCategoria::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = AscensoEscalaCategoria::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
