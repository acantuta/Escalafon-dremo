...
use App\Models\Escalafon\EducacionTipoCentro;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionTipoCentroController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionTipoCentro::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionTipoCentro::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduTipCentNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = EducacionTipoCentro::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionTipoCentro::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
