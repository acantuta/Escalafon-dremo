...
use App\Models\Escalafon\VMantenimientoEvaluacionDesempenioMotivoAccion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEvaluacionDesempenioMotivoAccionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEvaluacionDesempenioMotivoAccion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEvaluacionDesempenioMotivoAccion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cEvalDescAccMotNombre' => 'required|string|max:150',
            'iEvalDesAccId' => 'nullable|integer',
            'cRegLabNombre' => 'nullable|string|max:300',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEvaluacionDesempenioMotivoAccion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEvaluacionDesempenioMotivoAccion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
