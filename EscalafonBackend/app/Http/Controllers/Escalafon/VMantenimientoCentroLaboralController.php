...
use App\Models\Escalafon\VMantenimientoCentroLaboral;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoCentroLaboralController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoCentroLaboral::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoCentroLaboral::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iCentLabId' => 'required|integer',
            'iDirRegId' => 'required|integer',
            'iInstGeEduId' => 'required|integer',
            'iModEduId' => 'required|integer',
            'iNivEduId' => 'required|integer',
            'cCentLabCodigoModular' => 'required|string|max:50',
            'cCentLabNombre' => 'required|string|max:250',
            'bCentLabEsVigente' => 'required',
            'cDirRegNombre' => 'nullable|string|max:250',
            ...
            DB::beginTransaction();
            $resource = VMantenimientoCentroLaboral::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoCentroLaboral::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
