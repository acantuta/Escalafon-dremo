...
use App\Models\Escalafon\Provincia;
...
class ProvinciaController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = Provincia::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cPrvnCodigo' => 'nullable|string|max:10',
            'cPrvnNombre' => 'nullable|string|max:100',
            'cPrvnAbreviatura' => 'nullable|string|max:10',
            'iPrvnEstado' => 'nullable|integer',
            'cPrvnGentilicio' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = Provincia::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = Provincia::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
