...
class VInfopefamiliarDomicilioController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VInfopefamiliarDomicilio::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VInfopefamiliarDomicilio::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iSecInfoPeFamDomiId' => 'required|integer',
            'iLegId' => 'required|integer',
            'iTipDirId' => 'nullable|integer',
            'iZonaId' => 'nullable|integer',
            'iTipViaId' => 'nullable|integer',
            'iDptoId' => 'nullable|integer',
            'iPrvnId' => 'nullable|integer',
            'iDsttId' => 'nullable|integer',
            'cInfoPeFamDireccion' => 'nullable|string|max:150',
            'cInfoPeFamReferencia' => 'nullable|string|max:150',
            'iArchId' => 'nullable|integer',
            'cTipViaNombre' => 'nullable|string|max:100',
            'cZonaNombre' => 'nullable|string|max:100',
            'cTipDirNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = VInfopefamiliarDomicilio::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VInfopefamiliarDomicilio::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
