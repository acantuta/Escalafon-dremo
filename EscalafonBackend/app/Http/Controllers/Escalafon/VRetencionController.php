...
class VRetencionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VRetencion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VRetencion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cRetenNumeroDocumento' => 'required|string|max:250',
            'dtRetenFechaDocumento' => 'required|date',
            'iArchId' => 'required|integer',
            'iTipRetenId' => 'required|integer',
            'iComTipMonId' => 'nullable|integer',
            'nRetenMontoTotal' => 'required|numeric',
            'nRetenNumeroCuotas' => 'required|numeric',
            'nRetenMontoFijoMensual' => 'nullable|numeric',
            'nRetenPorcentajeFijoMensual' => 'nullable|numeric',
            'cRetenEntidadNombre' => 'nullable|string|max:300',
            'iTipBenRetenId' => 'nullable|integer',
            'cRetenNombreBeneficiario' => 'nullable|string|max:300',
            'cRetenAnotaciones' => 'nullable|string',
            'cTipRetenNombre' => 'nullable|string|max:100',
            'iRetenId' => 'required|integer',
            'iLegId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VRetencion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VRetencion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
