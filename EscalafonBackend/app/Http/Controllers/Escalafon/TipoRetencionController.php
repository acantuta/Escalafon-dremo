...
use App\Models\Escalafon\TipoRetencion;
...
class TipoRetencionController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoRetencion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipRetenNombre' => 'required|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = TipoRetencion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoRetencion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
