...
use App\Models\Escalafon\VMantenimientoCeseMotivoAccion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoCeseMotivoAccionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoCeseMotivoAccion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoCeseMotivoAccion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iCesMotAccId' => 'required|integer',
            'cCesAccNombre' => 'required|string|max:150',
            'cRegLabNombre' => 'nullable|string|max:300',
            'cCesMotAccNombre' => 'required|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoCeseMotivoAccion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoCeseMotivoAccion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
