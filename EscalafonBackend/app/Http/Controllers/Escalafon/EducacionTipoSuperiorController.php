...
use App\Models\Escalafon\EducacionTipoSuperior;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionTipoSuperiorController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionTipoSuperior::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionTipoSuperior::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduTipSupNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = EducacionTipoSuperior::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionTipoSuperior::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
