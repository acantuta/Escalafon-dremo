...
use App\Models\Escalafon\EducacionNivelEspecialidad;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionNivelEspecialidadController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionNivelEspecialidad::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionNivelEspecialidad::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iEduNivEdId' => 'nullable|integer',
            'iEduTipSupId' => 'nullable|integer',
            'iEduGradAlcId' => 'nullable|integer',
            'cEduNivEspNombre' => 'nullable|string|max:300',

            ]);

            DB::beginTransaction();
            $resource = EducacionNivelEspecialidad::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionNivelEspecialidad::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
