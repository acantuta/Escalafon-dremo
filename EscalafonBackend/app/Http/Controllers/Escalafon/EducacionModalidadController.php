...
use App\Models\Escalafon\EducacionModalidad;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionModalidadController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionModalidad::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionModalidad::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduModaNombre' => 'nullable|string|max:150',

            ]);

            DB::beginTransaction();
            $resource = EducacionModalidad::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionModalidad::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
