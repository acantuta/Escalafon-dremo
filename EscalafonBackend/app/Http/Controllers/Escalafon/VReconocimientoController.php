...
class VReconocimientoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VReconocimiento::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VReconocimiento::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cRecoNumeroDocumento' => 'required|string|max:150',
            'dtRecoFechaDocumento' => 'required|date',
            'iRecoTipMerId' => 'required|integer',
            'iArchId' => 'required|integer',
            'iRecoMerId' => 'required|integer',
            'cRecoEntidadEmisora' => 'required|string|max:300',
            'cRecoAnotaciones' => 'required|string',
            'cRecoTipMerNombre' => 'nullable|string|max:150',
            'cRecoMerNombre' => 'nullable|string|max:150',
            'dtRecoFechaInicio' => 'required|date',
            'cTipoDocNombre' => 'nullable|string|max:250',
            'iRecoId' => 'required|integer',
            'iLegId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VReconocimiento::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VReconocimiento::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
