...
use App\Models\Escalafon\VMantenimientoEducacionProgramaGeneral;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEducacionProgramaGeneralController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEducacionProgramaGeneral::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEducacionProgramaGeneral::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cEduCarrNombre' => 'nullable|string|max:150',
            'cEduProGenNombre' => 'required|string|max:150',
            'iEduCarrId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEducacionProgramaGeneral::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEducacionProgramaGeneral::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
