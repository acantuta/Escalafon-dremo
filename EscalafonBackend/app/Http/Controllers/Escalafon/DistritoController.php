...
use App\Models\Escalafon\Distrito;
...
class DistritoController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = Distrito::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cDsttCodigo' => 'nullable|string|max:10',
            'cDsttNombre' => 'nullable|string|max:100',
            'cDsttAbreviatura' => 'nullable|string|max:10',
            'iDsttEstado' => 'nullable|integer',
            'cDsttGentilicio' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = Distrito::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = Distrito::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
