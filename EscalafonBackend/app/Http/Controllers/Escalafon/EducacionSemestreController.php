...
use App\Models\Escalafon\EducacionSemestre;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class EducacionSemestreController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = EducacionSemestre::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = EducacionSemestre::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cEduSemNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = EducacionSemestre::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = EducacionSemestre::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
