...
class VExperienciaLaboralController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VExperienciaLaboral::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VExperienciaLaboral::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'iExpLabSecId' => 'required|integer',
            'cExpLabEntidad' => 'required|string|max:300',
            'cExpLabCargo' => 'required|string|max:250',
            'cExpLabNumeroDocumento' => 'required|string|max:150',
            'dtExpLabFechaDocumento' => 'required|date',
            'dtExpLabFechaInicio' => 'nullable|date',
            'dtExpLabFechaFin' => 'nullable|date',
            'cExpLabAnotaciones' => 'nullable|string',
            'iArchId' => 'required|integer',
            'cExpLabSecNombre' => 'nullable|string|max:50',
            'cExpLabFuncionesDesempenadas' => 'nullable|string',
            'iExpLabId' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VExperienciaLaboral::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VExperienciaLaboral::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
