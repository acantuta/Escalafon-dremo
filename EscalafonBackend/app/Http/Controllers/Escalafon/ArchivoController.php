...
class ArchivoController extends Controller
{
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = Archivo::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iArchFolios' => 'required|integer',
            'cArchExtension' => 'nullable|string|max:10',
            'cArchUuid' => 'nullable|string|max:36',

            ]);

            DB::beginTransaction();
            $resource = Archivo::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = Archivo::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
