...
use App\Models\Escalafon\CompensacionTipoFallecido;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class CompensacionTipoFallecidoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = CompensacionTipoFallecido::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = CompensacionTipoFallecido::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cComTipFallNombre' => 'required|string|max:50',

            ]);

            DB::beginTransaction();
            $resource = CompensacionTipoFallecido::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = CompensacionTipoFallecido::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
