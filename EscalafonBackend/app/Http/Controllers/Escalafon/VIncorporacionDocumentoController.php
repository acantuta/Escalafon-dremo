...
class VIncorporacionDocumentoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VIncorporacionDocumento::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VIncorporacionDocumento::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cIncorTipDocNombre' => 'nullable|string|max:250',
            'iIncorDocId' => 'required|integer',
            'iArchId' => 'required|integer',
            'iLegId' => 'required|integer',
            'iIncorTipDocId' => 'required|integer',
            'cIncorDocAnotaciones' => 'nullable|string',
            'dIncorDocFechEmision' => 'nullable|date',

            ]);

            DB::beginTransaction();
            $resource = VIncorporacionDocumento::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VIncorporacionDocumento::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
