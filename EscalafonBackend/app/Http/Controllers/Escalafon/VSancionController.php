...
class VSancionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VSancion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VSancion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            ...
            'cSancNumeroDocumento' => 'required|string|max:150',
            'cSancInstitucionEmiteDocumento' => 'required|string|max:300',
            'dtSancFechaDocumento' => 'required|date',
            'cSancAnotaciones' => 'nullable|string',
            'dtSancFechaFin' => 'nullable|date',
            'dtSancFechaInicio' => 'nullable|date',
            'dtSancFechaNotificacion' => 'nullable|date',
            'cSancCausaMotivo' => 'required|string|max:300',
            'iTipSancId' => 'required|integer',
            'iArchId' => 'nullable|integer',
            'cTipoDocNombre' => 'nullable|string|max:250',
            'iSancId' => 'required|integer',
            'iLegId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VSancion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VSancion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
