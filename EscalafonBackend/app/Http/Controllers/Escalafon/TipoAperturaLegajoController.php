...
use App\Models\Escalafon\TipoAperturaLegajo;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class TipoAperturaLegajoController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = TipoAperturaLegajo::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoAperturaLegajo::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipoAperLegNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = TipoAperturaLegajo::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoAperturaLegajo::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
