...
use App\Models\Escalafon\VCondicionLaboralSituacionLaboral;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VCondicionLaboralSituacionLaboralController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VCondicionLaboralSituacionLaboral::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VCondicionLaboralSituacionLaboral::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iCondSitId' => 'required|integer',
            'iCondLabId' => 'required|integer',
            'iSitLabId' => 'required|integer',
            'cSitLabNombre' => 'required|string|max:300',
            'cCondLabNombre' => 'nullable|string|max:50',
            'Expr1' => 'required|integer',

            ]);

            DB::beginTransaction();
            $resource = VCondicionLaboralSituacionLaboral::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VCondicionLaboralSituacionLaboral::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
