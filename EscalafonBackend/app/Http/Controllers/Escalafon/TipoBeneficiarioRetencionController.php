...
use App\Models\Escalafon\TipoBeneficiarioRetencion;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class TipoBeneficiarioRetencionController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = TipoBeneficiarioRetencion::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = TipoBeneficiarioRetencion::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'cTipBenRetenNombre' => 'nullable|string|max:100',

            ]);

            DB::beginTransaction();
            $resource = TipoBeneficiarioRetencion::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = TipoBeneficiarioRetencion::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
