...
class VInfopefamiliarDeclaracionJuradaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VInfopefamiliarDeclaracionJurada::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VInfopefamiliarDeclaracionJurada::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iSecInfDecId' => 'required|integer',
            'iLegId' => 'required|integer',
            'iInfoPeFamDecTipId' => 'required|integer',
            'dtInfoPeFamDecFechaEmision' => 'nullable|date',
            'cInfoPeFamDecAnotaciones' => 'nullable|string',
            'iArchivoId' => 'nullable|integer',
            'cInfoPeFamDecTipNombre' => 'required|string|max:800',

            ]);

            DB::beginTransaction();
            $resource = VInfopefamiliarDeclaracionJurada::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VInfopefamiliarDeclaracionJurada::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
