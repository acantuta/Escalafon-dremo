...
use App\Models\Escalafon\VMantenimientoEscalaCategoria;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;
use Illuminate\Support\Facades\DB;

class VMantenimientoEscalaCategoriaController extends Controller
{

    public function index(Request $request)
    {
        try {    
            $query = VMantenimientoEscalaCategoria::query();
    ...
    public function listarPaginado(Request $request)
    {
        try {    
            ...
            $result = VMantenimientoEscalaCategoria::fromStoredProcedure(
                $filters,
                $page,
                $pageSize,
                $sortColumn,
    ...
    public function store(Request $request)
    {
        try {
            $validatedData = $request->validate([
                'iEscCatId' => 'required|integer',
            'cRegLabNombre' => 'nullable|string|max:300',
            'cEscCatNombre' => 'required|string|max:150',
            'iRegLabId' => 'nullable|integer',

            ]);

            DB::beginTransaction();
            $resource = VMantenimientoEscalaCategoria::create($validatedData);
            DB::commit();
    ...
    {
        try {
            $resource = VMantenimientoEscalaCategoria::findOrFail($id);
            return response()->json([
                'success' => true,
                'data' => $resource
            ]);
